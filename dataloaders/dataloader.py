...
class NYUDataset(torch.utils.data.Dataset):
    ...
    def _read_rle(rle_filename):  # 0.0005s
        ...
        vox_rle = np.squeeze(vox_rle)  # 2d array: (1 x N), to 1d array: (N , ) (95546,)
        fid.close()
...
if __name__ == '__main__':
    ...
    # data_dir = '/data/elon/NYU_Raw/depthbin/NYUtrain'
    ...
    # ------------------------------------------------ #
    data_set = NYUDataset(data_dir)
    data_loader = torch.utils.data.DataLoader(dataset=NYUDataset(data_dir),
                                              batch_size=1,
                                              shuffle=False,
                                              num_workers=1)
    ...
    rgb_tensor, depth_tensor,tsdf_hr,target_lr_T, position,  _filename = data_set[500]
    print(type(rgb_tensor))
    print(type(_filename))
    print("rgb_tensor = {}".format(rgb_tensor.shape))
    rgb_tensor = rgb_tensor.swapaxes(0,2)
    rgb_tensor = rgb_tensor.swapaxes(0,1)
     
    rgb_tensor[:,:,0] = (rgb_tensor[:,:,0]*0.2023 + 0.4914)
    rgb_tensor[:,:,1] = (rgb_tensor[:,:,1]*0.1994+ 0.4822)
    rgb_tensor[:,:,2] = (rgb_tensor[:,:,2]*0.2010 + 0.4465)
    rgb_tensor = rgb_tensor*255
    
    print("rgb_tensor = {}".format(rgb_tensor.shape))
    # cv2.imwrite('out.jpg',np.uint8(rgb_tensor))
    im = Image.fromarray(np.uint8(rgb_tensor))
    im.save('out.jpg')
    # print("rgb_tensor = {}".format(rgb_tensor.shape))

    print("depth_tensor = {}".format(depth_tensor.shape))
    depth_tensor = depth_tensor.swapaxes(0,2)
    depth_tensor = depth_tensor.swapaxes(0,1)
    depth_tensor = np.asarray(depth_tensor*8000, np.uint16)
    print('depth_tensor = {}'.format(depth_tensor.shape))
    cv2.imwrite('depth.png',depth_tensor)
    # plt.imshow(depth_tensor ,cmap='Greys_r')
    # plt.savefig("depth.png")

    print("target_lr_T = {}".format(target_lr_T.shape))
    print(position)
    print("position = {}".format(position.shape))
    print("position_max = {}".format(position.max()))
    
    print('target_lr_T = {}'.format(target_lr_T.shape))
   
    target_lr_T_1 = target_lr_T.reshape(-1)
    np.savetxt('save_label.txt',target_lr_T_1, fmt='%d')
    data_set.labeled_voxel2ply(target_lr_T, './1.ply')

import numpy as np
import dataloaders.utils.fusion as fusion
import torch
...
def vox2pix(cam_E, cam_k, 
    ...
    -------
    projected_pix: (N, 2)
        Projected 2D positions of voxels 像素投影到图片上的坐标
    fov_mask: (N,)
        Voxels mask indice voxels inside image's FOV  表明像素点是否在视锥内
    pix_z: (N,)
        Voxels'distance to the sensor in meter 体素到相机原点的距离
    """
    # Compute the x, y, z bounding of the scene in meter 计算场景边界
    ...
    xv, yv, zv = np.meshgrid(range(vol_dim[0]),
                             range(vol_dim[1]),
                             range(vol_dim[2]),
                             indexing='ij')
    vox_coords = np.concatenate([xv.reshape(1,-1),
                                 yv.reshape(1,-1),
                                 zv.reshape(1,-1)], axis=0).astype(int).T

    # Project voxels'centroid from lidar coordinates to camera coordinates
    cam_pts = fusion.TSDFVolume.vox2world(vox_origin, vox_coords, voxel_size) # 将栅格体素投影到世界坐标系
    cam_pts = fusion.rigid_transform(cam_pts, cam_E) # 将世界坐标系投影到相机坐标系
...
def compute_local_frustums(projected_pix, pix_z, target, img_W, img_H, dataset, n_classes, size=4):
    ...
    # frustums_masks, frustums_class_dists = torch.from_numpy(frustums_masks), torch.from_numpy(frustums_class_dists)

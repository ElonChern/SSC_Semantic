...
def train():
    ...
    # optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, net.parameters()), lr=args.lr, weight_decay=0.0005, betas=(0.9, 0.999))
    ...
    for epoch in range(0, args.epochs):
        ...
        for step, (data, sequence,filename) in tqdm(enumerate(train_loader), desc=decs_str, unit='step'):
            # target should be a LongTensor. (bs, 60L, 36L, 60L)
            
            y_true_1 = data['target_1_1'].long().contiguous()
            y_true_2 = data['target_1_2'].long().contiguous()
            y_true_4 = data['target_1_4'].long().contiguous()
            y_true_1 = Variable(y_true_1.view(-1)).cuda()
            y_true_2 = Variable(y_true_2.view(-1)).cuda()
            y_true_4 = Variable(y_true_4.view(-1)).cuda()
            
            
            y_true = data['fov_target_1_4'].long().contiguous()
            ...
            x_depth = Variable(data['depth'].float()).cuda()
            
            data = dict_to(data, args.device, dtype = torch.float32)
            ...
            if args.model == 'palnet':
                ...
                x_rgb = Variable(data['img'].float()).cuda()
                y_pred, scores = net(x_depth=x_depth, x_rgb=x_rgb,data=data)
           
            y_pred = y_pred.permute(0, 2, 3, 4, 1).contiguous()  # (BS, C, D, H, W) --> (BS, D, H, W, C)
            y_pred = y_pred.view(-1, 20)  # C = 12
            
            y_pred_1 = scores['pred_semantic_1_1'].permute(0, 2, 3, 4, 1).contiguous()
            y_pred_1 = y_pred_1.view(-1, 20)
            y_pred_2 = scores['pred_semantic_1_2'].permute(0, 2, 3, 4, 1).contiguous()
            y_pred_2 = y_pred_2.view(-1, 20)  
            y_pred_4 = scores['pred_semantic_1_4'].permute(0, 2, 3, 4, 1).contiguous()
            y_pred_4 = y_pred_4.view(-1, 20)                      
            ...
            loss_2 = (loss_func(y_pred_1, y_true_1) + loss_func(y_pred_2, y_true_2) + loss_func(y_pred_4, y_true_4))/3
            total_loss = 0.2*loss + 0.8*loss_2
            total_loss.backward()
            optimizer.step()
            # break
...
def validate_on_dataset_stsdf(model, date_loader, save_ply=False):
    ...
    with torch.no_grad():
        ...
        for step, (data, sequence,filename) in tqdm(enumerate(date_loader), desc='Validating', unit='frame'):
            ...
            y_true_2 = data['target_1_2'].long().contiguous()
            y_true_4 = data['target_1_4'].long().contiguous()
            nonempty_1 = data['nonempty_1_1']
            nonempty_2 = data['nonempty_1_2']
            nonempty_4 = data['nonempty_1_4']
            ...
            if args.model == 'palnet':
                ...
                data = dict_to(data, args.device, dtype = torch.float32)
                y_pred, scores = model(x_depth=var_x_depth, x_rgb=var_x_rgb, data=data)  # y_pred.size(): (bs, C, W, H, D)

            # y_pred = y_pred.cpu().data.numpy()  # CUDA to CPU, Variable to numpy
            # y_true = y_true.cpu().data.numpy()
            # nonempty = nonempty.numpy()
            
            y_pred = scores['pred_semantic_1_1'].cpu().data.numpy()  # CUDA to CPU, Variable to numpy
            y_true = y_true_1.cpu().data.numpy()
           
            nonempty = nonempty_1.numpy()            
